...
int panfrost_mmu_map(struct panfrost_gem_mapping *mapping)
{
	...
	mmu_map_sg(pfdev, mapping->mmu, mapping->mmnode.start << PAGE_SHIFT, prot, sgt);
	mapping->active = true;
...
void panfrost_mmu_unmap(struct panfrost_gem_mapping *mapping)
{
	...
	dev_dbg(pfdev->dev, "unmap: as=%d, iova=%llx, len=%zx", mapping->mmu->as, iova, len);
	...
	panfrost_mmu_flush_range(pfdev, mapping->mmu, mapping->mmnode.start << PAGE_SHIFT, len);
