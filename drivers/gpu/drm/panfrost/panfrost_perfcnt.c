 ...
 * otherwise.
 */
static bool panfrost_perfcnt_job_ctx_eq(struct panfrost_perfcnt_job_ctx *a,
...
static bool panfrost_perfcnt_try_reuse_last_job_ctx(struct panfrost_job *job)
{
	...
	if (!panfrost_perfcnt_job_ctx_eq(prev_ctx, new_ctx)) {
	...
	job->perfcnt_fence = prev_ctx->wait_fence;
	return true;
}
