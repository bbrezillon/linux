...
static u16 hantro_h264_get_ref_nbr(struct hantro_ctx *ctx, unsigned int dpb_idx)
{
...
void rk3399_vpu_h264_dec_run(struct hantro_ctx *ctx)
{
	...
	u32 reg, dpb_valid = 0, dpb_longterm = 0;
	dma_addr_t addr;
	unsigned int offset = MV_OFFSET_420, i;
	...
	reg = VDPU_REG_REFER_VALID_E(dpb_valid);
	...
	reg = VDPU_REG_CH_QP_OFFSET2(pps->second_chroma_qp_index_offset) |
	      ...
	      VDPU_REG_PIC_MB_WIDTH(MB_WIDTH(ctx->dst_fmt.width));
	...
	if (ctrls->slices[0].flags & V4L2_H264_SLICE_FLAG_BOTTOM_FIELD)
		addr += ALIGN(ctx->dst_fmt.width, MB_DIM);
	...
	addr += offset * MB_WIDTH(ctx->dst_fmt.width) *
		   MB_HEIGHT(ctx->dst_fmt.height);
	if (ctrls->slices[0].flags & V4L2_H264_SLICE_FLAG_BOTTOM_FIELD)
		addr += 32 * MB_WIDTH(ctx->dst_fmt.width) *
			   MB_HEIGHT(ctx->dst_fmt.height);
