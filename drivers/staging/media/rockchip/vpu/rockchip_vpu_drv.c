...
dec_queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
{
	...
	src_vq->drv_priv = ctx;
	src_vq->ops = &rockchip_vpu_dec_src_queue_ops;
	...
	dst_vq->ops = &rockchip_vpu_dec_dst_queue_ops;
...
enc_queue_init(void *priv, struct vb2_queue *src_vq, struct vb2_queue *dst_vq)
{
	...
	src_vq->io_modes = VB2_MMAP | VB2_DMABUF;
	src_vq->drv_priv = ctx;
	src_vq->ops = &rockchip_vpu_enc_src_queue_ops;
	src_vq->mem_ops = &vb2_dma_contig_memops;
	...
	dst_vq->io_modes = VB2_MMAP | VB2_DMABUF;
	dst_vq->drv_priv = ctx;
	dst_vq->ops = &rockchip_vpu_enc_dst_queue_ops;
