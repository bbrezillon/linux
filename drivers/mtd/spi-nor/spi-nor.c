...
static int s3an_sr_ready(struct spi_nor *nor)
{
	...
	ret = spi_nor_read_reg(nor, SPINOR_OP_XRDSR, &val, 1);
	if (ret < 0) {
...
static inline int spi_nor_sr_ready(struct spi_nor *nor)
{
	...
	if (nor->flags & SNOR_F_USE_CLSR && sr & (SR_E_ERR | SR_P_ERR)) {
		...
		spi_nor_write_reg(nor, SPINOR_OP_CLSR, NULL, 0);
		return -EIO;
	}
...
static inline int spi_nor_fsr_ready(struct spi_nor *nor)
{
	...
	if (fsr & (FSR_E_ERR | FSR_P_ERR)) {
		...
		spi_nor_write_reg(nor, SPINOR_OP_CLFSR, NULL, 0);
		return -EIO;
	}
			     ...
			     size_t len, void *buf)
{
	...
	while (len) {
		ret = spi_nor_read_data(nor, addr, len, (u8 *)buf);
		if (!ret || ret > len) {
			ret = -EIO;
			goto read_err;
		}
...
static int spi_nor_probe(struct spi_mem *spimem)
{
	 ...
	 * We'll reallocate a new buffer if nor->page_size < PAGE_SIZE
	 * (which shouldn't happen before long since NOR page is usually less
	...
	if (nor->page_size > PAGE_SIZE) {
		...
		devm_kfree(&spi->dev, nor->bouncebuf);
		nor->bouncebuf = devm_kmalloc(&spi->dev, nor->bouncebuf_size,
					      GFP_KERNEL);
		if (nor->bouncebuf)
			return -ENOMEM;
